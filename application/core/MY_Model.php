...
class MY_Model extends CI_Model {
	...
	function get_task($id) {
        $sql = 'SELECT tb_daily_report.*, tb_task.name
				FROM tb_daily_report
				INNER JOIN tb_task
				ON tb_daily_report.task_id=tb_task.id
				WHERE tb_task.id ='.$id.'';
	...
	function get_project($id) {
        $sql = 'SELECT tb_project.*, tb_project_user.user_id
				FROM tb_project
				INNER JOIN tb_project_user
				ON tb_project.id=tb_project_user.project_id
				WHERE tb_project_user.user_id ='.$id.'';
    	$result = $this->db->query($sql);
    	return $result->result();
	}

	function get_pm_name($id){
		$i =  $this->get_column('tb_user', 'full_name',$where=array('id'=>$id));
		if($i) return $i;
		else return null;
	}
	
	function get_username_employee($userid){
		$i =  $this->get_column('tb_user', 'username',$where=array('id'=>$userid));
		if($i) return $i;
		else return null;		
	...
	function get_employees($id){
        $sql = 'SELECT tb_employee.*, tb_user.id
				FROM tb_employee
				INNER JOIN tb_user
				ON tb_employee.user_id=tb_user.id
